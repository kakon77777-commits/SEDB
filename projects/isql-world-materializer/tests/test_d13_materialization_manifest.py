...
from isql_core.semantic_addressing import SemanticAddressProfile  # proves merged ISQL dependency is usable
...
class D13MaterializationManifestTests(unittest.TestCase):
    def _commitment(self, root: Path, name: str, content: bytes, chunk_size: int):
        ...
        return source, sidecar, commitment

    def test_manifest_binds_real_d9_world_head_and_sorts_artifacts(self):
        # Keep one real ISQL object in the test path so the cross-project stack is not a fake fixture-only import.
        self.assertEqual(SemanticAddressProfile("p", "1").profile_id, "p")
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            _, a_sidecar, a = self._commitment(root, "a", b"A" * 32, 8)
            _, z_sidecar, z = self._commitment(root, "z", b"Z" * 48, 16)
            world = _world_head()
            ...
            self.assertEqual(manifest.world_head_manifest_sha256, world.manifest_sha256())
            verification = verify_materialization_manifest(
                manifest,
                expected_manifest_sha256=manifest.manifest_sha256,
                expected_world_head_manifest_sha256=world.manifest_sha256(),
                proof_indexes={
                    "asset:a": RangeProofIndex(a_sidecar),
                    "asset:z": RangeProofIndex(z_sidecar),
                },
            )
            self.assertTrue(verification.valid)

    def test_manifest_identity_is_deterministic_across_input_order(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            _, a_sidecar, a = self._commitment(root, "a", b"alpha" * 8, 8)
            _, b_sidecar, b = self._commitment(root, "b", b"beta" * 8, 8)
            world_hash = _world_head().manifest_sha256()
            one = build_materialization_manifest(
                world_id="world:alpha",
                world_head_manifest_sha256=world_hash,
                profile_id="p",
                commitments={"b": b, "a": a},
            )
            two = build_materialization_manifest(
                world_id="world:alpha",
                world_head_manifest_sha256=world_hash,
                profile_id="p",
                commitments={"a": a, "b": b},
    ...
    def test_extra_local_proof_cache_does_not_invalidate_manifest(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            _, a_sidecar, a = self._commitment(root, "a", b"A" * 24, 8)
            _, extra_sidecar, _ = self._commitment(root, "extra", b"E" * 24, 8)
            world_hash = _world_head().manifest_sha256()
            manifest = build_materialization_manifest(
                world_id="world:alpha",
                world_head_manifest_sha256=world_hash,
                profile_id="p",
                commitments={"asset:a": a},
            )
            verification = verify_materialization_manifest(
                manifest,
                expected_manifest_sha256=manifest.manifest_sha256,
                ...
                proof_indexes={
                    "asset:a": RangeProofIndex(a_sidecar),
                    "cached:unrelated": RangeProofIndex(extra_sidecar),
                },
            )
            self.assertTrue(verification.valid)

    def test_wrong_manifest_or_world_head_hash_is_reported_separately(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            _, sidecar, commitment = self._commitment(root, "a", b"A" * 24, 8)
            world_hash = _world_head().manifest_sha256()
            manifest = build_materialization_manifest(
                world_id="world:alpha",
                world_head_manifest_sha256=world_hash,
                profile_id="p",
                commitments={"asset:a": commitment},
            )
            wrong_manifest = verify_materialization_manifest(
                manifest,
                expected_manifest_sha256="0" * 64,
                expected_world_head_manifest_sha256=world_hash,
                proof_indexes={"asset:a": RangeProofIndex(sidecar)},
            )
            self.assertFalse(wrong_manifest.valid)
            self.assertFalse(wrong_manifest.manifest_hash_valid)
            self.assertTrue(wrong_manifest.world_head_binding_valid)
            wrong_world = verify_materialization_manifest(
                manifest,
                expected_manifest_sha256=manifest.manifest_sha256,
                expected_world_head_manifest_sha256="1" * 64,
                proof_indexes={"asset:a": RangeProofIndex(sidecar)},
            )
            self.assertFalse(wrong_world.valid)
            self.assertTrue(wrong_world.manifest_hash_valid)
            self.assertFalse(wrong_world.world_head_binding_valid)

    def test_wrong_proof_sidecar_cannot_satisfy_binding(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            _, a_sidecar, a = self._commitment(root, "a", b"A" * 24, 8)
            _, b_sidecar, _ = self._commitment(root, "b", b"B" * 24, 8)
            world_hash = _world_head().manifest_sha256()
            manifest = build_materialization_manifest(
                world_id="world:alpha",
                world_head_manifest_sha256=world_hash,
            ...
            verification = verify_materialization_manifest(
                ...
                expected_manifest_sha256=manifest.manifest_sha256,
                expected_world_head_manifest_sha256=world_hash,
                proof_indexes={"asset:a": RangeProofIndex(b_sidecar)},
            )
            self.assertFalse(verification.valid)
            self.assertFalse(verification.artifact_commitments_valid)

    def test_changing_proof_layout_changes_materialization_not_source_or_world(self):
    ...
    def test_verified_reader_hands_trusted_binding_to_d12_fetcher(self):
        content = bytes(range(64))
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            _, sidecar, commitment = self._commitment(root, "asset", content, 16)
            ...
            catalog.register(
                PlacementRecord(
                    identity=commitment.source_identity,
                    placement_id="local-copy",
                    provider_id="local",
            ...
            manifest = build_materialization_manifest(
                ...
                world_head_manifest_sha256=world_hash,
                profile_id="p",
                commitments={"asset:main": commitment},
            )
            reader = MaterializationRangeReader(
                ...
                range_fetcher=VerifiedRangeFetcher(
                    ...
                    {"local": LocalDirectoryProvider(provider_root)},
                ),
            )
    ...
    def test_reader_rejects_untrusted_manifest_hash_before_fetch(self):
        content = b"trust-boundary" * 4
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            _, sidecar, commitment = self._commitment(root, "asset", content, 8)
            world_hash = _world_head().manifest_sha256()
            manifest = build_materialization_manifest(
                world_id="world:alpha",
                world_head_manifest_sha256=world_hash,
                profile_id="p",
                commitments={"asset": commitment},
            )
            with self.assertRaises(MaterializationError) as ctx:
                MaterializationRangeReader(
                    ...
                    range_fetcher=VerifiedRangeFetcher(
                        PhysicalPlacementResolver(PlacementCatalog(root / "placements.sqlite3")),
                        {},
                    ),
                )
